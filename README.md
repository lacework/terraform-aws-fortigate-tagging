...
1. Create a FortiCNAPP alert channel with **Amazon CloudWatch** as the
   destination, using the ARN of the event bus this configuration creates.
2. Create an alert rule bound to that channel.
3. On the FortiGate, create a dynamic address object filtered by
   `fcnappalert=true` and reference it from a policy.
...
| `publisher_account_ids` | FortiCNAPP publishing accounts matched by the rule | `["434813966438"]` |

## Known limitation

Tagging depends on the alert payload carrying the EC2 instance ID. Not all alert
types include it. If an instance is not tagged, check the Lambda's CloudWatch log
group to confirm whether the payload contained an instance ID.
